...
pub const Pir = struct {
    ...
    pub fn alloc(allocator: anytype, linearized: Linearized, op_used: *u32) !Pir {
        ...
        if (op_num == 0) {
            ...
            const group_num_max: u32 = @truncate(@divFloor(linearized.op_num - (op_num + op_start), op_num) + 1);
        ...
        for (0..op_num) |op_idx| {
            ...
            for (0..repeat_num) |group_idx_usize| {
                const group_idx: u32 = @truncate(group_idx_usize);
                if (a_out_left) {
                    if (!a_out_reenter) {
