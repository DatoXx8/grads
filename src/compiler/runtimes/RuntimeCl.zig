...
pub const ClState = struct {
    device: ClDevice,
    context: ClContext,
    queue: ClCommandQueue,
    pub fn init(this: *anyopaque) ?void {
        var state: *ClState = @alignCast(@ptrCast(this));
    ...
    pub fn deinit(this: *anyopaque) ?void {
        const state: *ClState = @alignCast(@ptrCast(this));
...
pub fn memoryFree(this: *anyopaque, memory: Memory) ?void {
    _ = this;
    if (opencl.clReleaseMemObject(memory) != 0) {
        @branchHint(.cold);
        return null;
    }
}
pub fn memorySyncToHost(state: *anyopaque, mem: Memory, mem_host: *anyopaque, n_bytes: u32) ?void {
...
pub fn memorySyncToDevice(state: *anyopaque, mem: Memory, mem_host: *anyopaque, n_bytes: u32) ?void {
...
pub fn programAlloc(state: *anyopaque, source: []const u8) ?ProgramPtr {
    var err: i32 = 0;
...
pub fn programFree(state: *anyopaque, program: ProgramPtr) ?void {
    _ = state;
    if (opencl.clReleaseProgram(program) != 0) {
        @branchHint(.cold);
        return null;
    }
}
pub fn programLog(state: *anyopaque, program: ProgramPtr, allocator: Allocator) ?void {
...
pub fn kernelAlloc(state: *anyopaque, program: ProgramPtr, name: [*:0]const u8, args: Args) ?KernelPtr {
    _ = state;
    var err: i32 = 0;
    const kernel: opencl.cl_kernel = opencl.clCreateKernel(program, name, &err);
    if (err != 0) {
        @branchHint(.cold);
    ...
    for (0..args.arg_mem.len) |arg_idx| {
        ...
        if (opencl.clSetKernelArg(kernel.kernel, @intCast(arg_idx), //
...
pub fn kernelFree(state: *anyopaque, kernel: KernelPtr) ?void {
    _ = state;
    if (opencl.clReleaseKernel(kernel.ptr) != 0) {
        @branchHint(.cold);
        return null;
...
pub fn kernelRun(state: *anyopaque, kernel: KernelPtr, size_global: usize, size_local: usize) ?void {
    if (opencl.clEnqueueNDRangeKernel(state.queue, kernel.ptr, 1, null, //
...
pub fn queueWait(state: *anyopaque) ?void {
