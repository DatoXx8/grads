...
void simple_op_simulate_move(op_t *op) {
    ...
    switch(op->type_move) {
        ...
        case move_offset: {
            op->buffer_out->off_sim = op->buffer_out->str_a_sim * op->var_a + op->buffer_out->str_z_sim * op->var_z + op->buffer_out->str_y_sim * op->var_y +
                                      op->buffer_out->str_x_sim * op->var_x;
    ...
    switch(simple->type) {
        case operation_unary: {
            switch(simple->type_unary) {
                case unary_add: {
                    printf("U add {%lu, %lu, %lu, %lu} %lu [%lu, %lu, %lu, %lu] %lf %s\n", simple->buffer_out.sze_a, simple->buffer_out.sze_z,
                           simple->buffer_out.sze_y, simple->buffer_out.sze_x, simple->buffer_out.off, simple->buffer_out.off_a, simple->buffer_out.off_z,
                           simple->buffer_out.off_y, simple->buffer_out.off_x, simple->var_unary, simple->buffer_out.name);
                    break;
                }
                case unary_subtract: {
            ...
            switch(simple->type_binary) {
                case binary_add: {
                    printf("B add {%lu, %lu, %lu, %lu} %lu [%lu, %lu, %lu, %lu] < {%lu, %lu, %lu, %lu} %lu [%lu, %lu, %lu, %lu] %s %s\n",
                           simple->buffer_out.sze_a, simple->buffer_out.sze_z, simple->buffer_out.sze_y, simple->buffer_out.sze_x, simple->buffer_out.off,
                           simple->buffer_out.off_a, simple->buffer_out.off_z, simple->buffer_out.off_y, simple->buffer_out.off_x, simple->buffer_in.sze_a,
                           simple->buffer_in.sze_z, simple->buffer_in.sze_y, simple->buffer_in.sze_x, simple->buffer_in.off, simple->buffer_in.off_a,
                           simple->buffer_in.off_z, simple->buffer_in.off_y, simple->buffer_in.off_x, simple->buffer_out.name, simple->buffer_in.name);
                    break;
                }
                    ...
                    for(int64_t a = 0; a < simple->buffer_out.sze_a; a++) {
                        for(int64_t z = 0; z < simple->buffer_out.sze_z; z++) {
                            for(int64_t y = 0; y < simple->buffer_out.sze_y; y++) {
                                for(int64_t x = 0; x < simple->buffer_out.sze_x; x++) { SIMPLE_AT(simple->buffer_out, a, z, y, x) += simple->var_unary; }
                            }
                        }
                    }
                    break;
                        ...
                        for(int64_t z = 0; z < simple->buffer_out.sze_z; z++) {
                            for(int64_t y = 0; y < simple->buffer_out.sze_y; y++) {
                                for(int64_t x = 0; x < simple->buffer_out.sze_x; x++) {
                                    SIMPLE_AT(simple->buffer_out, a, z, y, x) = exp(SIMPLE_AT(simple->buffer_out, a, z, y, x));
                                }
                            }
                        }
                    }
                ...
                case unary_log: {
                    for(int64_t a = 0; a < simple->buffer_out.sze_a; a++) {
                        for(int64_t z = 0; z < simple->buffer_out.sze_z; z++) {
                            for(int64_t y = 0; y < simple->buffer_out.sze_y; y++) {
                                for(int64_t x = 0; x < simple->buffer_out.sze_x; x++) {
                                    SIMPLE_AT(simple->buffer_out, a, z, y, x) = log(SIMPLE_AT(simple->buffer_out, a, z, y, x));
                                }
                            }
                        }
                    }
                ...
                case unary_square: {
                    for(int64_t a = 0; a < simple->buffer_out.sze_a; a++) {
                        for(int64_t z = 0; z < simple->buffer_out.sze_z; z++) {
                            for(int64_t y = 0; y < simple->buffer_out.sze_y; y++) {
                                for(int64_t x = 0; x < simple->buffer_out.sze_x; x++) {
                                    SIMPLE_AT(simple->buffer_out, a, z, y, x) *= SIMPLE_AT(simple->buffer_out, a, z, y, x);
                                }
                            }
                        }
                    }
                ...
                case unary_sqrt: {
                    for(int64_t a = 0; a < simple->buffer_out.sze_a; a++) {
                        for(int64_t z = 0; z < simple->buffer_out.sze_z; z++) {
                            for(int64_t y = 0; y < simple->buffer_out.sze_y; y++) {
                                for(int64_t x = 0; x < simple->buffer_out.sze_x; x++) {
                                    SIMPLE_AT(simple->buffer_out, a, z, y, x) = sqrt(SIMPLE_AT(simple->buffer_out, a, z, y, x));
                                }
                            }
                        }
                    }
                ...
                case unary_reciprocal: {
                    for(int64_t a = 0; a < simple->buffer_out.sze_a; a++) {
                        for(int64_t z = 0; z < simple->buffer_out.sze_z; z++) {
                            for(int64_t y = 0; y < simple->buffer_out.sze_y; y++) {
                                for(int64_t x = 0; x < simple->buffer_out.sze_x; x++) {
                                    SIMPLE_AT(simple->buffer_out, a, z, y, x) = 1 / SIMPLE_AT(simple->buffer_out, a, z, y, x);
                                }
                            }
                        }
                    }
                ...
                case unary_tanh: {
                    for(int64_t a = 0; a < simple->buffer_out.sze_a; a++) {
                        for(int64_t z = 0; z < simple->buffer_out.sze_z; z++) {
                            for(int64_t y = 0; y < simple->buffer_out.sze_y; y++) {
                                for(int64_t x = 0; x < simple->buffer_out.sze_x; x++) {
                                    SIMPLE_AT(simple->buffer_out, a, z, y, x) = tanh(SIMPLE_AT(simple->buffer_out, a, z, y, x));
                                }
                            }
                        }
                    }
                ...
                case unary_absolute: {
                    for(int64_t a = 0; a < simple->buffer_out.sze_a; a++) {
                        for(int64_t z = 0; z < simple->buffer_out.sze_z; z++) {
                            for(int64_t y = 0; y < simple->buffer_out.sze_y; y++) {
                                for(int64_t x = 0; x < simple->buffer_out.sze_x; x++) {
                                    SIMPLE_AT(simple->buffer_out, a, z, y, x) = fabs(SIMPLE_AT(simple->buffer_out, a, z, y, x));
                                }
                            }
                        }
                    }
                ...
                case binary_add: {
                    ...
                    for(int64_t a = 0; a < simple->buffer_out.sze_a; a++) {
                        for(int64_t z = 0; z < simple->buffer_out.sze_z; z++) {
                            for(int64_t y = 0; y < simple->buffer_out.sze_y; y++) {
                                for(int64_t x = 0; x < simple->buffer_out.sze_x; x++) {
                                    SIMPLE_AT(simple->buffer_out, a, z, y, x) += SIMPLE_AT(simple->buffer_in, a, z, y, x);
                                }
                            }
                        }
                    }
                ...
                case binary_max: {
                    ...
                    for(int64_t a = 0; a < simple->buffer_out.sze_a; a++) {
                        for(int64_t z = 0; z < simple->buffer_out.sze_z; z++) {
                            for(int64_t y = 0; y < simple->buffer_out.sze_y; y++) {
                                for(int64_t x = 0; x < simple->buffer_out.sze_x; x++) {
                                    if(SIMPLE_AT(simple->buffer_out, a, z, y, x) < SIMPLE_AT(simple->buffer_in, a, z, y, x)) {
                                        SIMPLE_AT(simple->buffer_out, a, z, y, x) = SIMPLE_AT(simple->buffer_in, a, z, y, x);
                                    }
                                }
                            }
                ...
                case binary_add_like: {
                    ...
                    for(int64_t a = 0; a < simple->buffer_out.sze_a; a++) {
                        for(int64_t z = 0; z < simple->buffer_out.sze_z; z++) {
                            for(int64_t y = 0; y < simple->buffer_out.sze_y; y++) {
                                for(int64_t x = 0; x < simple->buffer_out.sze_x; x++) {
                                    SIMPLE_AT(simple->buffer_out, a, z, y, x) += SIMPLE_AT(simple->buffer_in, 0, 0, 0, 0);
                                }
                            }
                        }
                    }
                ...
                case binary_max_like: {
                    ...
                    for(int64_t a = 0; a < simple->buffer_out.sze_a; a++) {
                        for(int64_t z = 0; z < simple->buffer_out.sze_z; z++) {
                            for(int64_t y = 0; y < simple->buffer_out.sze_y; y++) {
                                for(int64_t x = 0; x < simple->buffer_out.sze_x; x++) {
                                    if(SIMPLE_AT(simple->buffer_out, a, z, y, x) < SIMPLE_AT(simple->buffer_in, 0, 0, 0, 0)) {
                                        SIMPLE_AT(simple->buffer_out, a, z, y, x) = SIMPLE_AT(simple->buffer_in, 0, 0, 0, 0);
                                    }
                                }
                            }
            ...
            switch(simple->type_reduce) {
                    ...
                    for(int64_t a = 0; a < simple->buffer_in.sze_a; a++) {
                        for(int64_t z = 0; z < simple->buffer_in.sze_z; z++) {
                            for(int64_t y = 0; y < simple->buffer_in.sze_y; y++) {
                                for(int64_t x = 0; x < simple->buffer_in.sze_x; x++) { temp += SIMPLE_AT(simple->buffer_in, a, z, y, x); }
                            }
                        }
                    }
                        ...
                        for(int64_t z = 0; z < simple->buffer_in.sze_z; z++) {
                            for(int64_t y = 0; y < simple->buffer_in.sze_y; y++) {
                                for(int64_t x = 0; x < simple->buffer_in.sze_x; x++) {
                                    if(temp < SIMPLE_AT(simple->buffer_in, a, z, y, x)) { temp = SIMPLE_AT(simple->buffer_in, a, z, y, x); }
                                }
                            }
                        }
                    }
                ...
                case reduce_avg: {
                    ...
                    SIMPLE_AT(simple->buffer_out, 0, 0, 0, 0) =
                        temp / (simple->buffer_in.sze_x * simple->buffer_in.sze_y * simple->buffer_in.sze_z * simple->buffer_in.sze_a);
                    break;
                }
                case reduce_min: {
                    double temp = INFINITY;
