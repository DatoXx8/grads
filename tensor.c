...
void op_single_print(op_t *op, int padding, int offset, const char *name) {
    ...
    //                        op->buffer_out->sze_y, op->buffer_out->sze_x, op->buffer_out->off, op->buffer_in->sze_a, op->buffer_in->sze_z,
    //                        op->buffer_in->sze_y, op->buffer_in->sze_x, op->buffer_in->off, (void *) op->buffer_out, (void *) op->buffer_in);
    //                 break;
    //             }
    ...
    //                        op->buffer_out->sze_y, op->buffer_out->sze_x, op->buffer_out->off, op->buffer_in->sze_a, op->buffer_in->sze_z,
    //                        op->buffer_in->sze_y, op->buffer_in->sze_x, op->buffer_in->off, (void *) op->buffer_out, (void *) (void *) op->buffer_in);
    //                 break;
    //             }
    //             case reduce_avg: {
    ...
    //                        op->buffer_out->sze_y, op->buffer_out->sze_x, op->buffer_out->off, op->var_a, op->var_z, op->var_y, op->var_x,
    //                        op->buffer_out->off, (void *) op->buffer_out);
    //                 break;
    //             }
    //             case move_resize: {
    ...
    //                        op->buffer_out->sze_y, op->buffer_out->sze_x, op->buffer_out->off, op->buffer_out->sze_a, op->buffer_out->sze_z,
    //                        op->buffer_out->sze_y, op->buffer_out->sze_x,
    //                        op->buffer_out->str_a * op->var_a + op->buffer_out->str_z * op->var_z + op->buffer_out->str_y * op->var_y +
    ...
    switch(op->type) {
            ...
            switch(op->type_unary) {
                case unary_add: {
                    printf("U add {%lu, %lu, %lu, %lu} %lu %lf [%p]\n", op->buffer_out->sze_a_sim, op->buffer_out->sze_z_sim, op->buffer_out->sze_y_sim,
                           op->buffer_out->sze_x_sim, op->buffer_out->off_sim, op->var_unary, (void *) op->buffer_out);
                    break;
                }
                case unary_subtract: {
                    printf("U sub {%lu, %lu, %lu, %lu} %lu %lf [%p]\n", op->buffer_out->sze_a_sim, op->buffer_out->sze_z_sim, op->buffer_out->sze_y_sim,
                ...
                case unary_exp: {
                    printf("U exp {%lu, %lu, %lu, %lu} %lu [%p]\n", op->buffer_out->sze_a_sim, op->buffer_out->sze_z_sim, op->buffer_out->sze_y_sim,
                           op->buffer_out->sze_x_sim, op->buffer_out->off_sim, (void *) op->buffer_out);
                    break;
                }
                case unary_log: {
                    printf("U log {%lu, %lu, %lu, %lu} %lu [%p]\n", op->buffer_out->sze_a_sim, op->buffer_out->sze_z_sim, op->buffer_out->sze_y_sim,
            ...
            switch(op->type_binary) {
                case binary_add: {
                    printf("B add {%lu, %lu, %lu, %lu} %lu < {%lu, %lu, %lu, %lu} %lu [%p] [%p]\n", op->buffer_out->sze_a_sim, op->buffer_out->sze_z_sim,
                           op->buffer_out->sze_y_sim, op->buffer_out->sze_x_sim, op->buffer_out->off_sim, op->buffer_in->sze_a_sim, op->buffer_in->sze_z_sim,
                           op->buffer_in->sze_y_sim, op->buffer_in->sze_x_sim, op->buffer_in->off, (void *) op->buffer_out, (void *) op->buffer_in);
                    break;
                }
                case binary_subtract: {
                    printf("B add {%lu, %lu, %lu, %lu} %lu < {%lu, %lu, %lu, %lu} %lu [%p] [%p]\n", op->buffer_out->sze_a_sim, op->buffer_out->sze_z_sim,
        ...
        case operation_reduce: {
            switch(op->type_reduce) {
                case reduce_sum: {
                    printf("R sum {%lu, %lu, %lu, %lu} %lu < {%lu, %lu, %lu, %lu} %lu [%p] [%p]\n", op->buffer_out->sze_a_sim, op->buffer_out->sze_z_sim,
                           op->buffer_out->sze_y_sim, op->buffer_out->sze_x_sim, op->buffer_out->off_sim, op->buffer_in->sze_a_sim, op->buffer_in->sze_z_sim,
                           op->buffer_in->sze_y_sim, op->buffer_in->sze_x_sim, op->buffer_in->off, (void *) op->buffer_out, (void *) (void *) op->buffer_in);
                    break;
                }
                case reduce_avg: {
                    printf("R avg {%lu, %lu, %lu, %lu} %lu < {%lu, %lu, %lu, %lu} %lu [%p] [%p]\n", op->buffer_out->sze_a_sim, op->buffer_out->sze_z_sim,
        ...
        case operation_move: {
            switch(op->type_move) {
                case move_reshape: {
                    printf("M rsp {%lu, %lu, %lu, %lu} %lu > {%lu, %lu, %lu, %lu} %lu [%p]\n", op->buffer_out->sze_a_sim, op->buffer_out->sze_z_sim,
                           op->buffer_out->sze_y_sim, op->buffer_out->sze_x_sim, op->buffer_out->off_sim, op->var_a, op->var_z, op->var_y, op->var_x,
                ...
                case move_offset: {
                    printf("M off {%lu, %lu, %lu, %lu} %lu > {%lu, %lu, %lu, %lu} %lu [%p]\n", op->buffer_out->sze_a_sim, op->buffer_out->sze_z_sim,
                           op->buffer_out->sze_y_sim, op->buffer_out->sze_x_sim, op->buffer_out->off_sim, op->buffer_out->sze_a_sim, op->buffer_out->sze_z_sim,
                           op->buffer_out->sze_y_sim, op->buffer_out->sze_x_sim,
                           op->buffer_out->str_a_sim * op->var_a + op->buffer_out->str_z_sim * op->var_z + op->buffer_out->str_y_sim * op->var_y +
    ...
    switch(op->type) {
                    ...
                    for(int64_t a = 0; a < op->buffer_out->sze_a; a++) {
                        for(int64_t z = 0; z < op->buffer_out->sze_z; z++) {
                            for(int64_t y = 0; y < op->buffer_out->sze_y; y++) {
                                for(int64_t x = 0; x < op->buffer_out->sze_x; x++) { BUFFER_AT_(op->buffer_out, a, z, y, x) += op->var_unary; }
                            }
                        }
                    }
                    break;
                            ...
                            for(int64_t y = 0; y < op->buffer_out->sze_y; y++) {
                                for(int64_t x = 0; x < op->buffer_out->sze_x; x++) {
                                    BUFFER_AT_(op->buffer_out, a, z, y, x) =
                                        BUFFER_AT_(op->buffer_out, a, z, y, x) > op->var_unary ? BUFFER_AT_(op->buffer_out, a, z, y, x) : op->var_unary;
                                }
                            }
                        }
                    }
        ...
        case operation_reduce: {
            switch(op->type_reduce) {
                    ...
                    for(int64_t a = 0; a < op->buffer_in->sze_a; a++) {
                        for(int64_t z = 0; z < op->buffer_in->sze_z; z++) {
                            for(int64_t y = 0; y < op->buffer_in->sze_y; y++) {
                                for(int64_t x = 0; x < op->buffer_in->sze_x; x++) { temp += BUFFER_AT_(op->buffer_in, a, z, y, x); }
                            }
                        }
                    }
                        ...
                        for(int64_t z = 0; z < op->buffer_in->sze_z; z++) {
                            for(int64_t y = 0; y < op->buffer_in->sze_y; y++) {
                                for(int64_t x = 0; x < op->buffer_in->sze_x; x++) {
                                    if(temp < BUFFER_AT_(op->buffer_in, a, z, y, x)) { temp = BUFFER_AT_(op->buffer_in, a, z, y, x); }
                                }
                            }
                        }
                    }
                ...
                case reduce_avg: {
                    ...
                    BUFFER_AT_(op->buffer_out, 0, 0, 0, 0) = temp / (op->buffer_in->sze_a * op->buffer_in->sze_a * op->buffer_in->sze_a * op->buffer_in->sze_a);
                    break;
                }
                case reduce_min: {
                    assert(op->buffer_out->sze_a == 1);
        ...
        case operation_move: {
            switch(op->type_move) {
                ...
                case move_offset: {
                    op->buffer_out->off = op->buffer_out->str_a * op->var_a + op->buffer_out->str_z * op->var_z + op->buffer_out->str_y * op->var_y +
                                          op->buffer_out->str_x * op->var_x;
                    break;
                }
            }
    ...
    for(int64_t a = 0; a < tensor->buffer->sze_a; a++) {
        ...
        for(int64_t z = 0; z < tensor->buffer->sze_z; z++) {
            ...
            for(int64_t y = 0; y < tensor->buffer->sze_y; y++) {
                ...
                for(int64_t x = 0; x < tensor->buffer->sze_x; x++) { printf("% lf ", BUFFER_AT_(tensor->buffer, a, z, y, x)); }
                printf("]\n");
            }
        }
    }
